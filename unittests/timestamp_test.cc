...
class TimestampTestCompareOperators : public ::testing::Test
{
 protected:
  ...
  virtual void SetUp()
  {
    Generator::TimestampGen::make_timestamp(&sample_timestamp, 2010, 9, 8, 0, 0, 0);
...
{
  Generator::DateGen::make_date(&before_sample_timestamp, 1980, 1, 1);
  Generator::DateGen::make_date(&identical_with_sample_timestamp, 2010, 9, 8);
  Generator::DateGen::make_date(&after_sample_timestamp, 2019, 5, 30);
}

template<> void TimestampTestCompareOperators<DateTime>::initBeforeIdenticalAfter()
{
  Generator::DateTimeGen::make_datetime(&before_sample_timestamp, 1990, 12, 31, 12, 12, 30);
  Generator::DateTimeGen::make_datetime(&identical_with_sample_timestamp, 2010, 9, 8, 0, 0, 0);
  Generator::DateTimeGen::make_datetime(&after_sample_timestamp, 2020, 4, 4, 4, 4, 4);
}

template<> void TimestampTestCompareOperators<Timestamp>::initBeforeIdenticalAfter()
{
  Generator::TimestampGen::make_timestamp(&before_sample_timestamp, 2010, 9, 7, 23, 59, 59);
  Generator::TimestampGen::make_timestamp(&identical_with_sample_timestamp, 2010, 9, 8, 0, 0, 0);
  Generator::TimestampGen::make_timestamp(&after_sample_timestamp, 2010, 9, 8, 0, 0, 1);
}
...
TEST_F(TimestampTest, is_valid_minOfTimestampRange_shouldReturn_True)
{
  ...
  Generator::TimestampGen::make_timestamp(&timestamp, year, month, day, hour, minute, second);
...
{
  time_t time;
  Generator::TimestampGen::make_timestamp(&timestamp, 2009, 6, 3, 4, 59, 1);
