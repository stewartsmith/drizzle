  ...
  *bufsz= 0;
  ...
  if (total_rows != HA_POS_ERROR)
  {
    ...
    *flags |= HA_MRR_USE_DEFAULT_IMPL;
    cost->zero();
    cost->setavg_io_cost(1); /* assume random seeks */
    if ((*flags & HA_MRR_INDEX_ONLY) && total_rows > 2)
      cost->setio_count(index_only_read_time(keyno, (uint32_t)total_rows));
    else
      cost->setio_count(read_time(keyno, n_ranges, total_rows));
    cost->setcpu_cost((double) total_rows / TIME_FOR_COMPARE + 0.01);
  }
  return total_rows;
}
                                   ...
                                   uint32_t *bufsz, uint32_t *flags, optimizer::COST_VECT *cost)
{
  ...
  *flags |= HA_MRR_USE_DEFAULT_IMPL;
  ...
  if (*flags & HA_MRR_INDEX_ONLY)
    cost->setio_count(index_only_read_time(keyno, n_rows));
  else
    cost->setio_count(read_time(keyno, n_ranges, n_rows));
  return 0;
}
