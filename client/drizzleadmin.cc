...
/* drizzleadmin command tool (based on the drizzle command line tool)
...
static bool ignore_errors= false, quick= false,
  ...
  output_tables= false, skip_updates= false,
...
# endif
  ...
  po::store(po::command_line_parser(argc, argv).options(long_options).
            style(style).extra_parser(parse_password_arg).run(),
            vm);
                 ...
                 uint32_t silent)
{
  ...
  if (drizzle_con_add_tcp(&drizzle, &con, (char *)host.c_str(),
    ...
    (char *)password.c_str(), (char *)database.c_str(), options) == NULL)
  {
...
*/
  ...
  if (connected)
  {
    ...
    */
    if (drizzle_query_str(&con,&result,"select USER() limit 1",
                          &ret) != NULL && ret == DRIZZLE_RETURN_OK &&
