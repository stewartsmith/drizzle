...
#endif
  if (!drizzle_init(&con_slot->drizzle))
    die("Failed on drizzle_init()");
...
int main(int argc, char **argv)
{
  ...
  if (drizzle_server_init(embedded_server_arg_count,
      embedded_server_args,
      (char**) embedded_server_groups))
    die("Can't initialize DRIZZLE server");
  ...
  cur_con= connections;
  if (!( drizzle_init(&cur_con->drizzle)))
    die("Failed in drizzle_init()");
  if (opt_compress)
