...
"master" servers to a "slave" server.  A slave is a server on which the
``slave`` plugin is loaded, configured, and ran.  Slaves can connect to up
to ten masters at once over the network (TCP/IP) and pair with the
