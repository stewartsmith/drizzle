	...
	ib_bh_t*	ib_bh)	/*!< in, own: UNDO log min binary heap */
{
	...
	purge_sys = (trx_purge_t*)mem_zalloc(sizeof(trx_purge_t));
...
/*===============================*/
	trx_purge_t*	purge_sys_instance)	/*!< in/out: purge instance */

{
	ulint		zip_size = 0;

	mutex_enter(&purge_sys_instance->bh_mutex);

	/* Only purge consumes events from the binary heap, user
	threads only produce the events. */

	if (!ib_bh_is_empty(purge_sys_instance->ib_bh)) {
		trx_rseg_t*	rseg;

		rseg = ((rseg_queue_t*) ib_bh_first(purge_sys_instance->ib_bh))->rseg;
		ib_bh_pop(purge_sys_instance->ib_bh);

		mutex_exit(&purge_sys_instance->bh_mutex);

		purge_sys_instance->rseg = rseg;
	} else {
		mutex_exit(&purge_sys_instance->bh_mutex);

		purge_sys_instance->rseg = NULL;

		return(ULINT_UNDEFINED);
	}

	ut_a(purge_sys_instance->rseg != NULL);

	mutex_enter(&purge_sys_instance->rseg->mutex);

	ut_a(purge_sys_instance->rseg->last_page_no != FIL_NULL);

	/* We assume in purge of externally stored fields
	that space id == 0 */
	ut_a(purge_sys_instance->rseg->space == 0);

	zip_size = purge_sys_instance->rseg->zip_size;

	ut_a(purge_sys_instance->purge_trx_no <= purge_sys_instance->rseg->last_trx_no);

	purge_sys_instance->purge_trx_no = purge_sys_instance->rseg->last_trx_no;

	purge_sys_instance->hdr_offset = purge_sys_instance->rseg->last_offset;

	purge_sys_instance->hdr_page_no = purge_sys_instance->rseg->last_page_no;

	mutex_exit(&purge_sys_instance->rseg->mutex);
	...
	ulint		zip_size)		/*!< in: block size or 0 */
{
	...
	purge_sys_instance->hdr_offset = purge_sys_instance->rseg->last_offset;
	page_no = purge_sys_instance->hdr_page_no = purge_sys_instance->rseg->last_page_no;

	if (purge_sys_instance->rseg->last_del_marks) {
		mtr_t		mtr;
		...
		undo_rec = trx_undo_get_first_rec(
			...
			purge_sys_instance->hdr_page_no,
			purge_sys_instance->hdr_offset, RW_S_LATCH, &mtr);
	...
	purge_sys_instance->offset = offset;
	purge_sys_instance->page_no = page_no;
	purge_sys_instance->purge_undo_no = undo_no;

	purge_sys_instance->next_stored = TRUE;
}
