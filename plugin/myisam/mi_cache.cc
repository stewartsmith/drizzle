...
int _mi_read_cache(IO_CACHE *info, unsigned char *buff, uint64_t pos, uint32_t length,
		   int flag)
{
  ...
  {
    read_length=length;
    if ((uint64_t) read_length > (uint64_t) (info->pos_in_file-pos))
  ...
  if (pos >= info->pos_in_file &&
      (offset= (uint64_t) (pos - info->pos_in_file)) <
      (uint64_t) (info->read_end - info->request_pos))
  {
