...
static int chk_index(MI_CHECK *param, MI_INFO *info,MI_KEYDEF *keyinfo,
		     uint64_t page, unsigned char *buff, ha_rows *keys,
...
extern "C"
{
  ...
  uint64_t get_record_for_key(MI_INFO *info,MI_KEYDEF *keyinfo,
                              unsigned char *key);
  int sort_insert_key(MI_SORT_PARAM  *sort_param,
                      register SORT_KEY_BLOCKS *key_block,
                      unsigned char *key, uint64_t prev_block);
...
int chk_del(MI_CHECK *param, register MI_INFO *info, uint32_t test_flag)
{
  ...
  uint64_t empty, next_link, old_link= 0;
  ...
  else
  {
	...
	goto wrong;
      ...
      {
	uint64_t prev_link=mi_sizekorr(buff+12);
...
int chk_size(MI_CHECK *param, register MI_INFO *info)
{
  ...
  if ((skr=(uint64_t) info->state->key_file_length) != size)
  {
  ...
  skr=(uint64_t) info->state->data_file_length;
  if (info->s->options & HA_OPTION_COMPRESS_RECORD)
    skr+= MEMMAP_EXTRA_MARGIN;
#ifdef USE_RELOC
  if (info->data_file_type == STATIC_RECORD &&
      skr < (uint64_t) info->s->base.reloc*info->s->base.min_pack_length)
    skr=(uint64_t) info->s->base.reloc*info->s->base.min_pack_length;
#endif
  if (skr != size)
  {
...
int chk_key(MI_CHECK *param, register MI_INFO *info)
{
  ...
  uint64_t all_keydata,all_totaldata,key_totlength,length;
       ...
       rec_per_key_part+=keyinfo->keysegs, key++, keyinfo++)
  {
    ...
    length=(uint64_t) isam_key_length(info,keyinfo)*keys + param->key_blocks*2;
    if (param->testflag & T_INFO && param->totaldata != 0L && keys != 0L)
      printf("Key: %2d:  Keyblocks used: %3d%%  Packed: %4d%%  Max levels: %2d\n",
	     key+1,
	     (int) (uint64_t2double(param->keydata)*100.0/uint64_t2double(param->totaldata)),
	     (int) ((uint64_t2double(length) - uint64_t2double(param->keydata))*100.0/
		    uint64_t2double(length)),
	     param->max_level);
  ...
  if (param->testflag & T_INFO)
  {
    if (all_totaldata != 0L && found_keys > 0)
      printf("Total:    Keyblocks used: %3d%%  Packed: %4d%%\n\n",
	     (int) (uint64_t2double(all_keydata)*100.0/
		    uint64_t2double(all_totaldata)),
	     (int) ((uint64_t2double(key_totlength) -
		     uint64_t2double(all_keydata))*100.0/
		     uint64_t2double(key_totlength)));
                     ...
                     ha_checksum *key_checksum, uint32_t level)
{
  ...
  if (page + keyinfo->block_length > info->state->key_file_length)
  {
    ...
    uint64_t max_length= lseek(info->s->kfile, 0, SEEK_END);
    mi_check_print_error(param, "Invalid key block position: %s  "
                         "key block size: %u  file_length: %s",
    ...
    info->state->key_file_length= (max_length &
                                   ~ (uint64_t) (keyinfo->block_length - 1));
  }
...
int chk_data_link(MI_CHECK *param, MI_INFO *info,int extend)
{
  ...
  uint64_t used, empty, pos, splits, start_recpos= 0,
...
static int writekeys(MI_SORT_PARAM *sort_param)
{
  ...
  uint64_t filepos= sort_param->filepos;
...
int movepoint(register MI_INFO *info, unsigned char *record, uint64_t oldpos,
...
int mi_sort_index(MI_CHECK *param, register MI_INFO *info, char * name)
{
  ...
  int new_file;
  uint64_t index_pos[HA_MAX_POSSIBLE_KEY];
			  ...
			  uint64_t pagepos, int new_file)
{
  ...
  uint64_t new_page_pos,next_page;
  char llbuff[22];
		      ...
		      const char * name, int rep_quick)
{
  ...
  uint64_t new_header_length,del;
  int new_file;
  MI_SORT_PARAM sort_param;
  MYISAM_SHARE *share=info->s;
  HA_KEYSEG *keyseg;
  ...
  {
    uint64_t skr=info->state->data_file_length+
      (share->options & HA_OPTION_COMPRESS_RECORD ?
                    ...
                    uint64_t prev_block)
{
  ...
  uint64_t filepos,key_file_length;
...
int test_if_almost_full(MI_INFO *info)
{
  if (info->s->options & HA_OPTION_COMPRESS_RECORD)
    return 0;
  return (uint64_t)(lseek(info->s->kfile, 0L, SEEK_END) / 10 * 9) >
         (uint64_t) info->s->base.max_key_file_length ||
         (uint64_t)(lseek(info->dfile, 0L, SEEK_END) / 10 * 9) >
         (uint64_t) info->s->base.max_data_file_length;
}
