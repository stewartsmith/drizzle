...
public:
  ...
  int close() { return 0; }
  int rnd_next(unsigned char*) { return -1; }
  int rnd_pos(unsigned char*, unsigned char*) { return -1; }
  void position(const unsigned char*) { return; }
  int info(uint32_t) { return 0; }
  void get_auto_increment(uint64_t, uint64_t, uint64_t, uint64_t*, uint64_t*) {}
  int doStartTableScan(bool) { return -1; }
};
