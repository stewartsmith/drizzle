  ...
  plugin::TableFunction("DATA_DICTIONARY", "SESSIONS")
{
  ...
  add_field("HOSTNAME", NI_MAXHOST);
  add_field("SCHEMA");
  add_field("COMMAND", 16);
  add_field("STATE_TIME", plugin::TableFunction::NUMBER, 0, false);
  add_field("STATE", plugin::TableFunction::STRING, 256, true);
  add_field("QUERY", plugin::TableFunction::STRING, PROCESS_LIST_WIDTH, true);
  add_field("THREAD_ID", plugin::TableFunction::STRING, 256, true);
}
...
bool Sessions::Generator::populate()
{
  ...
  while ((tmp= session_generator))
  {
    ...
    /* DRIZZLE_TIME */
    push(static_cast<uint64_t>(tmp->start_time ?  getSession().query_start() - tmp->start_time : 0));
